...
<div id="body">
	...
	<div id="wrap">
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<c:forEach items="${employments}" var="model">
				<tr>
					<td class="tableData">${model.id}</td>
					<td class="tableData-left-align">${model.hiredDate}</td>
					<td class="tableData-right-align">${model.terminationDate}</td>
					<td class="tableData-right-align">${model.employmentStatus.name}</td>
                    <td class="tableData-right-align">${model.paySchedule.paySchedule}</td>
					<td class="tableData">
						<a href="<%=request.getContextPath()%>/payroll/employees/${model.id}/edit">Edit</a>
					</td>
				</tr>
			</c:forEach>
		</table>
