						...
						$("#addNewLine")
								.click(
										function() {
											...
											var new_line = $('#expenseDetail_0')
													.clone();
											new_line = new_line.attr(
													"id",
													"expenseDetail_"
															+ lineItems).attr(
													"lineNum", lineItems);
											new_line
													.find("#remove-lineitem-0")
													.attr(
															"id",
															"remove-lineitem-"
																	+ lineItems)
											...
											new_line
													.find(
															"#expenseDetails0\\.expenseAccountId")
													.attr(
															"id",
															"expenseDetails"
																	+ lineItems
	...
	function resetIds() {
		$('.lineItemsClass')
				.each(
						function(index) {
							var value = parseInt($(this).attr('lineNum'));
							$(this).attr("id", "expenseDetail_" + index).attr(
									"lineNum", index);
							if (index == 0) {
							...
							$(this).find(
									"#expenseDetails" + value + "\\.productId")
									.attr(
											"id",
							...
							$(this).find(
									"#expenseDetails" + value
											+ "\\.description").attr("id",
									"expenseDetails" + index + ".description")
									.attr(
											"name",
											"expenseDetails[" + index
													+ "].description");
							$(this).find(
									"#expenseDetails" + value + "\\.quantity")
									.attr(
											"id",
											"expenseDetails" + index
													+ ".quantity").attr(
											...
											"expenseDetails[" + index
													+ "].quantity");
							$(this).find(
									"#expenseDetails" + value + "\\.unitPrice")
									.attr(
											"id",
											"expenseDetails" + index
													+ ".unitPrice").attr(
...
<div id="body">
  <div class="form-wrapper">
    <div class="form-big" style="width: 98%">
      ...
      <form:form action="processAdd" method="post" modelAttribute="expenseForm">
        <!-- print all errors if exists -->
        <spring:hasBindErrors name="expenseForm">
          <div class="response-msg error">
            <spring:bind path="expenseForm">
            ...
            <spring:bind path="expenseForm.*">
              <c:forEach items="${status.errors.fieldErrors}" var="error">
                ...
                <c:choose>
                  <c:when test="${fn:contains(theString, 'expenseForm.expenseDetail')}">
                  </c:when>
                  <c:otherwise>
        ...
        <table class="form">
          <tr>
            <td class="key" width="100px;">Vendor:</td>
            <td><form:select path="vendorId" cssClass="text" id="vendor">
          ...
          <tr>
            <td class="key">Voucher No:</td>
            <td><form:input path="purchaseOrder" /></td>
          </tr>
        </table>
        <table class="details">
          <thead>
          ...
          <c:forEach items="${expenseForm.expenseDetails}" varStatus="i">
            <tr id="expenseDetail_${i.index}" class="lineItemsClass" lineNum="${i.index}">
              <td><form:select path="expenseDetails[${i.index}].productId" cssStyle="width: 200px;">
                  <form:options items="${products}" itemLabel="name" itemValue="id" />
                </form:select></td>
              <td><form:select path="expenseDetails[${i.index}].expenseAccountId" cssStyle="width: 200px;">
